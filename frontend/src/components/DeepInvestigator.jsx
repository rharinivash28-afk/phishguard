...
export default function DeepInvestigator({ initialEmail, presets = [], onGenerateReport, onViewReport }) {
  ...
  const loadSample = (sample) => {
    setFormData({
      ...
      recipient: sample.recipient || 'you@example.com',
      body: sample.body,
      urls: sample.urls?.map(u => u.url).join('\n') || '',
      spf_status: sample.spf_status || 'UNKNOWN',
      dkim_status: sample.dkim_status || 'UNKNOWN',
      dmarc_status: sample.dmarc_status || 'UNKNOWN'
    });
    setResult(null);
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {(presets || []).map((sample) => (
            <button key={sample.id} onClick={() => loadSample(sample)}
              className="text-left p-3.5 rounded-xl glass-soft hover:bg-white/[0.09] transition border border-white/10 hover:border-white/25">
              <p className="text-sm font-bold text-white/90">{sample.title}</p>
              <p className="text-[11px] text-white/45 font-mono truncate mt-1">{sample.sender_address}</p>
              {sample.preset_note && (
                <p className="text-[11px] text-white/40 mt-2 leading-relaxed">{sample.preset_note}</p>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 glass p-5 space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-white/10">
            ...
            <span className="pill-muted font-mono">Offline · zero-PII</span>
          </div>

          <p className="text-[11px] text-white/45 leading-relaxed">
            Paste the header and body fields of a suspicious email, or click a preset above. Nothing is sent
            anywhere except the local scoring engine.
          </p>

          <div className="space-y-3">
            {field('Sender email address', 'sender_address', { mono: true, placeholder: 'security@paypa1-login.com' })}
            {field('Display name (as shown in the inbox)', 'display_name', { placeholder: 'PayPal Security Team' })}
            {field('Subject line', 'subject', { placeholder: 'Your account will be suspended!' })}
            {field('Links in the email — one URL per line', 'urls', { textarea: true, rows: 2, mono: true, placeholder: 'http://paypa1-login.com/verify' })}
          ...
          {!analysis ? (
            <div className="h-full min-h-[420px] flex flex-col items-center justify-center text-center p-8 border-2 border-dashed border-white/12 rounded-xl text-white/40">
              ...
              <p className="text-xs max-w-sm mt-1">
                Click a preset above or fill the form, then run the analysis to see multi-factor risk scoring,
                typosquat math, fired threat indicators, and a STIX 2.1 export.
              </p>
              {presets[0] && (
                <button onClick={() => loadSample(presets[0])} className="btn-ghost mt-4">
                  Load the {presets[0].title} preset
                </button>
              )}
            </div>
          ) : (
